...
Place your global OpenCode config in the global config directory. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

| Platform | Global config directory |
|---|---|
| **Linux** | `~/.config/opencode/` |
| **macOS** | `~/Library/Application Support/opencode/` |
| **Windows** | `%APPDATA%\opencode\` (usually `C:\Users\<you>\AppData\Roaming\opencode\`) |

For TUI-specific settings, add `tui.json` alongside `opencode.json` in the same directory.
...
**Quick bypass**: Set `OPENCODE_AUTO_PERMISSION=1` to skip all permission prompts entirely. All tools run immediately without asking — equivalent to `{"*": "allow"}`. Only use in trusted directories with git version control.

```bash
OPENCODE_AUTO_PERMISSION=1 opencode
```

You can also set it permanently in your shell profile:

```bash
export OPENCODE_AUTO_PERMISSION=1
```
